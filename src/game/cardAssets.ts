/**
 * Mapping des images de cartes espagnoles pour Okey
...
export function getCardImage(cardId: string): ReturnType<typeof require> {
  // Strip "-2" suffix for duplicate cards (Okey uses doubled deck)
  const displayId = cardId.replace(/-2$/, '');
  return CARD_IMAGES[displayId] || CARD_BACK;
}
