/**
 * @file auth/login.tsx
 * @description Login screen for the Okey app. Card image logo, email/password login, guest mode, and demo credentials.
...
const cardImage = require('../../assets/cards/5E.png');
...
export default function LoginScreen() {
  ...
  useEffect(() => {
    console.log('[Okey/LoginScreen] Component mounted');
  }, []);

  const handleLogin = async () => {
    if (!email || !password) {
      console.log('[Okey/LoginScreen] Validation failed: empty email or password');
      Alert.alert(t('error'), t('fillAllFields'));
      return;
    }
    setLoading(true);
    console.log('[Okey/LoginScreen] State update: loading = true');
    try {
      console.log('[Okey/LoginScreen] Fetching login...');
      await api.login(email, password, { gameType: 'okey' });
      console.log('[Okey/LoginScreen] Login successful');
      router.replace('/(tabs)');
    } catch (e: any) {
      console.error('[Okey/LoginScreen] Error:', e);
      Alert.alert(t('error'), e.message || t('loginError'));
    } finally {
      setLoading(false);
      console.log('[Okey/LoginScreen] State update: loading = false');
    }
  };
  ...
  const { promptAsync, ready: googleReady, missingClientId, missingNative } = useGoogleSignIn(
    async (idToken: string) => {
      setLoading(true);
      try {
        await api.loginWithGoogle(idToken, { gameType: 'okey' });
        console.log('[Okey/Login] Google login successful');
        router.replace('/(tabs)');
      } catch (e: any) {
        console.error('[Okey/Login] Google error:', e);
        Alert.alert(t('error'), e.message || 'Google sign-in failed');
      } finally {
        setLoading(false);
      }
    },
    (msg: string) => { console.warn('[Okey/Login] Google:', msg); setLoading(false); }
  );
  ...
  const handleGuest = async () => {
    ...
    console.log('[Okey/LoginScreen] State update: loading = true');
    try {
      console.log('[Okey/LoginScreen] Fetching guest session...');
      await api.createGuestSession();
      console.log('[Okey/LoginScreen] Guest session created');
      router.replace('/(tabs)');
    } catch (e: any) {
      console.error('[Okey/LoginScreen] Error:', e);
      ...
      <ScrollView contentContainerStyle={s.content} keyboardShouldPersistTaps="handled">
        ...
        <View style={s.form}>
          ...
          <TextInput
            ...
            value={email}
            onChangeText={(val) => {
              console.log('[Okey/LoginScreen] State update: email =', val);
              setEmail(val);
            }}
          ...
          <TextInput
            ...
            value={password}
            onChangeText={(val) => {
              console.log('[Okey/LoginScreen] State update: password = [hidden]');
              setPassword(val);
            }}
            placeholder="••••••••"
            placeholderTextColor="#6B7280"
...
const s = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#051a22' },
  content: {
    flexGrow: 1,
...
/* === End of auth/login.tsx — Okey — SallyCards === */
