 ...
 * @description Leaderboard Okey avec 3 portées : Mondial / Pays / Ville.
 * Données réelles depuis /leaderboards/okey?scope=<...>, auto-détection
...
export default function LeaderboardScreen() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      log.bin(`GET /leaderboards/okey`, { scope, filter });
      const r = await api.getLeaderboardScoped('okey', filter, scope, 50);
      log.bout('200 /leaderboards/okey', `${r.entries.length} entrées`);
