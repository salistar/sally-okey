/**
 * @file (tabs)/index.tsx
 * @description Home Okey. 4 actions principales : Créer, Rejoindre, vs Bot
...
export default function HomeScreen() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setUser(u);
      log.bin('GET /challenges/daily/okey');
      const ch = await api.getDailyChallenge('okey');
      if (ch) {
        log.bout('200 /challenges/daily/okey', { title: ch.title, reward: ch.rewardCoins });
        setChallenge(ch);
      }
  ...
  const handleChallenge = async () => {
    log.bin('POST /challenges/daily/okey/matchmake');
    try {
      const room = await api.joinDailyChallenge('okey');
