 ...
 * @description Profil Okey avec vraies valeurs (stats/coins/achievements/
...
export default function ProfileScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // 2) /leaderboards/okey/my-rank : best-effort
      try {
        log.bin('GET /leaderboards/okey/my-rank');
        const r = await api.getMyRank('okey', 'season');
        ...
        const msg = e?.message ?? e?.toString?.() ?? 'unknown';
        const status = e?.status ?? '?';
        if (status === 404) log.explain('pas encore de rang okey pour ce joueur');
